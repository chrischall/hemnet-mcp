 ...
 *
 * The read tools in this server hand back Hemnet's payload close to
 ...
 * honestly say which of Hemnet's fields matter and which are noise.
 *
 ...
 */
export const HN_VIEWS = ['compact', 'full'] as const;

const NOTE =
  'compact strips image/avatar URLs from the response; "full" returns Hemnet\'s payload untouched. ' +
  'No field projection: this server has no verified record of which Hemnet fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(HN_VIEWS, { note: NOTE });
...
export function viewResponse(view: string | undefined, data: unknown): ReturnType<typeof minifiedResult> {
  const rung: View = resolveView(view, HN_VIEWS);
